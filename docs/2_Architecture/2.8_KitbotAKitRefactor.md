...
An interface in Java is an abstract class. Interfaces define a list of methods that must be created in any class that implements that interface. Interfaces provide a unified template, whereby multiple classes can implement the same interface, and since the methods defined by the interface always have to be implemented, objects can be created from any class that implements an interface without breaking other code. This is commonly used in robot code to define Simulation and "real" versions of systems that can seamlessly be switched out. 
